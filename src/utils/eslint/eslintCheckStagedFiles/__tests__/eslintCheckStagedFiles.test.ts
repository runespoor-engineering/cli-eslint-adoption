...
describe("eslintCheckStagedFiles", () => {
	...
	it("should resolve when no staged files", async () => {
		...
		vi.spyOn(
			eslintCheckFilesModule,
			"eslintCheckFilePaths",
		).mockResolvedValue();
	...
	it("should use custom rootDir", async () => {
		...
		await expect(
			eslintCheckStagedFiles({ rootDir: customRootDir }),
	...
	it("should use custom filesRegex", async () => {
		...
		await expect(
			eslintCheckStagedFiles({ filesRegex: customRegex }),
		).resolves.toBeUndefined();
	...
	it("should propagate errors from eslintCheckFilePaths", async () => {
		...
		vi.spyOn(eslintCheckFilesModule, "eslintCheckFilePaths").mockRejectedValue(
			mockError,
		);
	...
	it("should handle multiple regex patterns", async () => {
		...
		await expect(
			eslintCheckStagedFiles({ filesRegex: multipleRegex }),
		).resolves.toBeUndefined();
